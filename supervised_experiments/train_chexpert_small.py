...
def train_multi_task(args, random_seed):
    ...
    task_labels = [int(i) for i in task_labels]
    tasks = configs[args.dataset]['tasks']
    tasks = [tasks[t] for t in task_labels]
    print("Tasks : ", tasks)
    ...
    train_loader, val_loader = datasets.get_dataset(args.dataset, args.batch_size, configs,
                                                    ...
                                                    partial_dataset=args.partial_dataset, chexphoto_labels=data_labels)
    ...
    torch.save({'stats': train_val_stats, 'args': vars(args)},
               f"{results_folder}{args.label}_{args.task_labels}_{run_name}_validation_results.pkl")
