    ...
    with torch.no_grad():
        ...
        for batch_val in test_loader:
            ...
            for idx, t in enumerate(tasks):
                out_t_val, _, _ = model[t](val_rep, None)
                loss_t = loss_fn[t](out_t_val, test_labels[:, int(t)])
                losses[t] += loss_t.item()  # for logging purposes
                metric[t].update(out_t_val, test_labels[:, int(t)])
            num_test_batches += 1
...
def train_multi_task(args, random_seed):
    ...
    if not args.debug:
        wandb.init(project="saliency_robustness", group=args.label, config=args, reinit=True)
        dropout_str = "" if not args.dropout else "-dropout"
        wandb.run.name = f"{args.nih_labels}_{args.optimizer}{dropout_str}-lr:{args.lr}-wd:{args.weight_decay}_" + wandb.run.name
    ...
    test_loader = datasets.get_dataset(args.dataset, args.test_batch_size, configs,
    ...
    model = model_selector.get_model(args.dataset, tasks, device=DEVICE,
    ...
    if args.optimizer == 'pcgrad':
        ...
        mtl_opt = RLW(optimizer, len(tasks), distribution=args.optimizer.split("-")[1])
    else:
    ...
    for epoch in range(args.num_epochs):
        ...
        epoch_stats[f"train_loss_avg"] = np.mean(torch.stack(list(losses_per_epoch.values())).cpu().detach().numpy())
        ...
        with torch.no_grad():
            ...
            for batch_val in val_loader:
                ...
                for idx, t in enumerate(tasks):
                    ...
                    loss_t = loss_fn[t](out_t_val, labels_val[:, int(t)])
                    losses_per_epoch[t] += loss_t.item()  # for logging purposes
                    metric[t].update(out_t_val, labels_val[:, int(t)])
                num_val_batches += 1
        ...
        epoch_stats[f"val_loss_avg"] = np.mean(np.array(loss_list))
        # Store aggregator metrics (e.g., avg) as well
        for agg_key in aggregators:
            ...
            if c_saver_metric[k] >= best_result[k]:
                ...
                if args.store_models:
                    ...
                    save_model(model, optimizer, scheduler, tasks, epoch, args,
                               folder=configs["utils"]["model_storage_old"], name=k)
    ...
    torch.save({'stats': train_val_stats, 'args': vars(args)},
               f"{results_folder}{args.label}_{args.nih_labels}_{run_name}_validation_results.pkl")
    # Save training/validation results.
    if args.store_models and (not args.time_measurement_exp):
        # Save last model.
        save_model(model, optimizer, scheduler, tasks, epoch, args, folder=configs["utils"]["model_storage_old"],
                   name="last")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='', help='which dataset to use',
                        choices=['celeba', 'mnist', 'cityscapes', 'nih', 'cov_nih'])
