...
def load_saved_model(models, tasks, net_basename, folder="saved_models/", name="last"):
    ...
    for batch_val in tqdm(test_loader):
        ...
        if test_labels[0][-1] == 1:
            ...
            rho_dict = ols_score(replace_extension(image_name[0]), gradient_t_orig.shape[0], \
                    configs[args.dataset]['lung_segment_path'], gradient_t_orig, gradient_t_corr, rho_dict, \
                    "Smooth")

            gradient_t_orig_sq = normalize_gradient(gradient_t_orig_sq)
            gradient_t_corr_sq = normalize_gradient(gradient_t_corr_sq)   
            rho_dict = ols_score(replace_extension(image_name[0]), gradient_t_orig.shape[0], \
                    configs[args.dataset]['lung_segment_path'], gradient_t_orig_sq, gradient_t_corr_sq, rho_dict, \
                    "SmoothSq") 
            # print(rho_dict)  
            df = pd.concat([df, pd.DataFrame([rho_dict])], ignore_index=True)
    filename = args.dataset + args.net_basename + "_" +args.model_type + ".csv"            
