...
def segmentation(input_image, unet,device,image_name, fileext,save_path):
    ...
    origin = torchvision.transforms.functional.resize(input_image, (256, 256))
    ...
    np_image = np.array(input_image.resize((256,256)), np.uint8)
...
def segmentation_func(image_path, segmentation_model, device):
    ...
    if 1:
        ...
        resized, lung_masked = segmentation(input_image, segmentation_model, device, seg_image_name, fileext, trial_save_lung)
